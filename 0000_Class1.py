...
# by using the functional testing we obtain the -> redundancy (meaning: we reduce the coding by just add all the code into the function.)
# by using the functional testing we also obtain the reusability (meaning: we use re-run the code again and again by just calling the funciton.)
...
# menaing; let's suppose we are creating / designing a Bukcet => so, in reality every bucket would have
#   the color, size, capacity., etc => these are called as the properties of that bucket
